...
// Iconos de línea (estilo lexflow): SVG stroke=currentColor. ponytail: set mínimo necesario.
...
@Component({
  selector: 'app-icon',
  template: `<span class="ic-svg" [style.width.px]="size()" [style.height.px]="size()" [innerHTML]="svg()"></span>`,
  styles: [`.ic-svg { display: inline-flex; align-items: center; justify-content: center; line-height: 0; }`],
})
export class Icon {
  private san = inject(DomSanitizer);
  name = input<string>('');
  size = input(18);
  svg = computed(() => this.san.bypassSecurityTrustHtml(
    `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="100%" height="100%">${ICONS[this.name()] ?? ''}</svg>`));
}
